...
const I18N = {
  ...
  exportGrid:'Exportando para rede',
  importGrid:'Consumindo da rede',

  homeConsume:'Consumo da casa',
  battLabel:'🔋 Bateria',

  etaFull:'⚡ Tempo estimado para carga completa: ',
  etaLeft:'🔁 Tempo restante estimado: ',

  slowCharge:'⚡ Carregamento muito lento',
  slowDisch:'🔁 Descarga muito lenta',
  ...
  savingStat:'Economia',
  systemStat:'⚙️ SISTEMA',
  statsTitle:'Estatísticas de hoje',

  solarProduced:'Produção Solar',
  solarUsed:'Consumo da Casa',

  forecastToday:'☀️ Previsão de hoje',
  forecastTmr:'☀️ Previsão de amanhã',
  actualToday:'☀️ Produção real hoje',

  peakForecast:'Previsão',
  actualNow:'Atual',
  ...
  ticker(ws,daily,dailyuse,savFmt,cur,bPct,isCharge,isDisch,battSoc){
    ...
    if(ws==='rainy'||ws==='pouring'){
      msg=`🌧️ Dia chuvoso, geração solar reduzida${
        kw>0 ? ' — produção de '+daily+' kWh' : ''
      }.`;
    }
    else if(ws==='lightning'){
      msg='⛈️ Tempestade detectada, geração solar reduzida por segurança.';
    }
    else if(ws==='cloudy'){
      msg=`☁️ Céu nublado, geração solar reduzida${
        kw>0 ? ' — '+daily+' kWh gerados' : ''
      }.`;
    }
    else if(ws==='partlycloudy'){
      msg=`⛅ Parcialmente nublado — gerou ${daily} kWh e a casa consumiu ${dailyuse} kWh.`;
    }
    else if(ws==='sunny'){
      if(kw>15){
        msg=`☀️ Excelente geração solar — ${daily} kWh produzidos e ${dailyuse} kWh consumidos!`;
      } else if(kw>8){
        msg=`☀️ Boa geração solar — ${daily} kWh produzidos e ${dailyuse} kWh consumidos.`;
      } else {
        msg=`☀️ Sol presente, porém geração ainda baixa — ${daily} kWh.`;
      }
    }
    else if(ws==='fog'){
      msg='🌫️ Neblina intensa reduzindo eficiência solar.';
    }
    else if(ws==='windy'){
      msg=`💨 Ventos fortes, geração solar normal — ${daily} kWh.`;
    }
    else{
      msg=`🌤️ Solar gerou ${daily} kWh, casa consumiu ${dailyuse} kWh — economia de ${savFmt}${cur}.`;
    }

    if(battSoc===100){
      msg+=' ✅ Bateria totalmente carregada!';
    }
    else if(isCharge && battSoc>80){
      msg+=` 🔋 Bateria carregando e quase cheia (${bPct}%).`;
    }
    else if(isDisch && battSoc<20){
      msg+=` ⚠️ Bateria baixa (${bPct}%)!`;
    }
...
class SolarWeatherCardEditor extends HTMLElement {
  ...
  _render(){
    ...
    const lang      = this._config.language||'pt';
    ...
    const E={
      ...
      pt:{
        displayOpts:'🎨 Opções de exibição',
        flowStyle:'🌊 Estilo do fluxo',
        language:'🌐 Idioma',
        bgOpacity:'🪟 Transparência do fundo',
        showWeatherInfo:'☁️ Mostrar painel do clima',
        showTomorrow:'🌤️ Mostrar previsão de amanhã',
        nodeGlow:'✨ Efeito brilho nos nós',
        showForecastChart:'📊 Mostrar gráfico solar',
        showHourlyStrip:'🕐 Mostrar previsão por hora',
        minimalMode:'🪄 Modo mínimo',
        pricingTitle:'💰 Preços e moeda',
        currencyLabel:'💱 Símbolo da moeda',
        pricingTiersLabel:'📊 Faixas de preço personalizadas',
        pricingHint:'Formato: limite_kWh:preço',
        layoutTitle:'📐 Layout dos nós',
        layoutHint:'Ajuste caso os nós sejam cortados em telas menores.',
        batNodeX:'🔋 Nó bateria X',
        grdNodeX:'🔌 Nó rede X',
        invNodeY:'⚙️ Nó inversor Y',
        homNodeY:'🏠 Nó casa Y',
        nodeScale:'📐 Escala dos nós',
        particleDensity:'✦ Densidade das partículas',
        gridOptions:'🔌 OPÇÕES DA REDE',
        homeOptions:'🏠 OPÇÕES DA CASA',
        timeFormat:'🕐 Formato de hora',
        showClock:'🕐 Mostrar relógio e clima',
        showRoomsSection:'🏠 Mostrar cômodos/dispositivos',
        showBatteryBar:'🔋 Mostrar barra da bateria',
        showStatsCircles:'📊 Mostrar círculos estatísticos',
        showBatteryNode:'🔋 Mostrar nó da bateria',
        tempUnit:'🌡️ Unidade de temperatura'
      },
      en:{
        ...
        <div class="opt-row">
          ...
          <div class="lang-grid" id="langGrid">
            ${[
             ...
             {v:'pt', flag:'<img src="https://flagcdn.com/16x12/br.png" width="20" height="14" alt="PT"/> Português (Brasil)'},
    ...
    if(partMSl){
      ...
      partMSl.addEventListener('change',e=>{
        this._config={...this._config,particle_mult:parseFloat(e.target.vaconst lang=cfg.languagelue)};
        this._fire();
      });
    }
...
const lang=cfg.language
  ...
  _render(){
    ...
    const cfg=this._config;
    if(!Object.values(cfg).some(v=>v&&String(v).trim()&&!['particle','wave','bubble','line','vi','pt','en','de','it','fr','nl','pl','sv','hu','12h','24h'].includes(v)&&isNaN(v)&&v!=='đ'&&v!=='€'&&v!=='$')){ this._unconfigured(); return; }
...
class SolarWeatherCard extends HTMLElement {
  ...
  static getStubConfig(){
    return {
      ...
      battery_voltage_entity:'', battery_capacity_entity:'', battery_capacity_wh:'',
      grid_flow_entity:'', grid_voltage_entity:'', grid_today_entity:'',
      home_consumption_entity:'', solar_today_entity:'', consumption_todayconst lang=cfg.language_entity:'',
      inverter_status_entity:'', inverter_switch_entity:'', grid_direct_entity:'',
      inverter_temp_entity:'', battery_temp_entity:'',
    };
  }
